...
Daten stammen vom OGD Datensatz. Siehe folgende URL fuer mehr Info
...
**Definition**: Sämtliche Summen, Anteile und Vergleiche basieren auf der **Anzahl betroffener Personen** (`AnzPersonWir` im Datensatz). Diese Variable gibt die Zahl der Individuen an, die eine Leerkündigung erhalten haben.
...
- `r WITHIN_CITY_CATEGORIES[1]` (gleiches Stadtquartier)
- `r WITHIN_CITY_CATEGORIES[2]` (anderes Stadtquartier)
...
Die globalen Chi-Quadrat-Tests in Abschnitt 5 deuten auf einen signifikanten Zusammenhang zwischen Altersgruppe und dem Zielort hin (sowohl für die binäre Klassifikation `innerhalb/ausserhalb der Stadt` als auch für alle Zielortkategorien). In diesem Abschnitt wird untersucht, welche spezifischen Zielorte zu den beobachteten Unterschieden beitragen.

## 5.1 Globaler Chi-Quadrat-Test (Alle Zielorte)

Um den Zusammenhang zwischen Altersgruppe und dem gesamten Spektrum der Zielortkategorien (inklusive aller ausserstädtischen Zielorte und "Unbekannt") zu untersuchen, führen wir einen weiteren Chi-Quadrat-Test auf der vollen Kontingenztafel Altersgruppe $\times$ Zielort durch.
...
# Resultate als Tabelle
chi_full_table <- tibble::tibble(
  Kennwert = c("Teststatistik (χ²)", "Freiheitsgrade", "p-Wert", "Cramer's V"),
  Wert = c(
    round(as.numeric(chi_full_result$statistic), 2),
    as.numeric(chi_full_result$parameter),
    format.pval(chi_full_result$p.value, digits = 3),
    round(v_full, 3)
  )
)
...
# kable(tab_full, caption = "Kontingenztafel: Altersgruppe × Alle Zielorte")
```


## 5.2 Standardisierte Residuen nach Altersgruppe und Zielort: Wo liegen die auffälligsten Abweichungen?
...
# 5.3 Relativrisiko nach Altersgruppe und Zielort: Quantifizierung von Über- und Unterrepräsentation
...
## 5.4 Verbleib im gleichen Stadtquartier
...
## Appendix A2 Total variations-Distanzmatrix: Wie ähnlich sind die Jahre bezüglich Zielort-Verteilung?
